    ...
    return {
        ...
        "app.py": "\n".join(
            [
                ...
                "STATE_DIR = Path(os.environ.get('LABFORGE_STATE_DIR', '/state'))",
    ...
    return {
        ...
        "app.py": "\n".join(
            [
                ...
                "",
                "",
                "def operations_reference_payload():",
                "    discovery = load_json('vulnerability-discovery.json', {'items': []})",
                "    clues = load_json('clues.json', {'items': []}).get('items', [])",
                "    workflow = load_json('workflow.json', {'service': SERVICE, 'routes': ROUTES, 'normal_workflows': []})",
                "    items = []",
                "    for item in discovery.get('items', []):",
                "        routes = item.get('normal_routes', []) or []",
                "        clues_for_item = item.get('natural_clues', []) or []",
                "        items.append({",
                "            'business_feature': item.get('business_feature'),",
                "            'normal_routes': routes,",
                "            'operator_language': item.get('operator_language'),",
                "            'discovery_cues': clues_for_item,",
                "            'noise': item.get('noise', []),",
                "            'next_observation': 'Confirm the normal route behavior first, then compare edge-case behavior against the documented workflow.',",
                "        })",
                "    return {",
                "        'service': SERVICE,",
                "        'purpose': PURPOSE,",
                ...
                "        'normal_workflows': workflow.get('normal_workflows', []),",
                "        'operations_notes': clues,",
                "        'items': items,",
                "        'policy': {",
                "            'learner_facing': True,",
                "            'intent': 'Business-shaped operational reference; it provides workflow context without final answers or exploit payloads.',",
                "        },",
                "    }",
                "",
                "",
                "@app.get('/operations/reference')",
                "def operations_reference():",
                "    append_event('operations.reference.viewed')",
                "    return jsonify(operations_reference_payload())",
                "",
                "",
                "@app.get('/api/clues')",
                "def api_clues():",
                ...
                "      {% if discovery.get('items') %}<section style=\"margin-top:18px\"><h2>Workflow Hints</h2>{% for item in discovery.get('items', []) %}<p class=\"muted\"><strong>{{ item.get('business_feature') }}</strong><br>{% for clue in item.get('natural_clues', [])[:2] %}{{ clue }}<br>{% endfor %}<span class=\"pill\">noise: {{ ', '.join(item.get('noise', [])[:2]) }}</span></p>{% endfor %}</section>{% endif %}",
...
def render_reset_script() -> str:
    return "\n".join(
        [
            "#!/usr/bin/env sh",
            "set -eu",
            "STATE_DIR=\"${LABFORGE_STATE_DIR:-/state}\"",
            "rm -rf \"$STATE_DIR/tmp\" 2>/dev/null || true",
            "rm -f \"$STATE_DIR/stage-state.json\" 2>/dev/null || true",
            "mkdir -p \"$STATE_DIR\"",
            "echo ok",
            "",
        ]
