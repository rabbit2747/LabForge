...
def command_services_agent_packages(args: argparse.Namespace) -> int:
    try:
        ...
        return 1
    spec = LabSpec.load(Path(args.lab))
    written = create_service_agent_packages(
        spec,
        Path(args.out),
        adapter=args.adapter,
        live_readiness_tasks_path=Path(args.live_readiness_tasks) if args.live_readiness_tasks else None,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    services_agent_packages_parser.add_argument(
        "--live-readiness-tasks",
        help="Optional live-readiness-tasks.json generated by the pipeline gate; included in service-builder packages.",
    )
