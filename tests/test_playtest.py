...
class PlaytestTests(unittest.TestCase):
    def test_playtest_generates_learner_access_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("live_readiness_requirements", access_bundle)
            ...
            requirement_status = {item["name"]: item["status"] for item in access_bundle["live_readiness_requirements"]}
            self.assertEqual(requirement_status["browser"], "declared")
            self.assertEqual(requirement_status["terminal"], "declared")
            self.assertEqual(requirement_status["persistent-tunnel"], "declared")
            self.assertEqual(requirement_status["stage-chain"], "declared")
            self.assertEqual(requirement_status["solver"], "declared")
            self.assertIn(requirement_status["plugin-evidence"], {"declared", "missing"})
            ...
            self.assertIn("Live Verification Requirements", access_bundle_md)
