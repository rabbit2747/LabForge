...
- Orchestrator LLM / specialist agent 역할 정의
- dry-run agent workspace
- agent task / output / decision artifact
- LabForge scenario review skill 후보
- LabForge MCP server 후보
- PR review static checks
- 라이선스/보안 노출 자동 검사

LLM/Agent 계층은 후반 자동화가 아니라 시나리오 제작 방식의 핵심 축으로 앞당긴다. 다만 실제 LLM 호출은 바로 붙이지 않고, 먼저 artifact 구조와 검증 규칙을 고정한다.
