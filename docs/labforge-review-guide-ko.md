...
- 모든 취약 서비스를 자동 생성하지는 않는다. 다만 지원 플러그인(`ssti-preview`, `stored-xss-review`, `idor-object-access`, `ssrf-internal-fetch`, `path-traversal-download`, `unsafe-file-upload`, `diagnostic-command-injection`, `build-pipeline-abuse`, `signed-update-publish`, `customer-update-callback`)은 `services materialize`로 안전한 Docker MVP runtime과 lab-scoped 취약 동작을 생성하고 실행 검증할 수 있다.
